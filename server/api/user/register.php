...
if(isset($request) && !empty($request)){
    ...
    switch (true) {
        case in_array($email,$usersEmailAddress):
            $data = [ 'status' => 303, 'data' => array("email" => $email,"password" => $password),'msg' => "Account already exists on this email!",'error' => 1 ];
            http_response_code(303);
            break;
        case in_array($username,$registeredUsers):
            $data = [ 'status' => 303, 'data' => array("email" => $email,"password" => $password),'msg' => "Username Is Exist Try another one",'error' => 1 ];
            http_response_code(303);
            break;
        case !in_array($username,$registeredUsers) && !in_array($email,$usersEmailAddress):
            ...
            $data = [ 'status' => 200, 'data' => array("userInfo" => $recentkyRegisteredUsers[0],"token" => $token ),'msg' => "Registered successful.",'error' => 0 ];
